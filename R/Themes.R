...
theme_pub <- function(base_size = 11,
	...
	if(legend_title)
	title_element <- element_text() else
	title_element <- element_blank()
	if(axis_lines)
	axis_line =  element_line(size = ifelse(axis_lines, grid::unit(lines_lwd, "mm"),0), color = "black") else
		axis_line <- element_blank()

		if(plot_box)
			panel_border <- ggplot2::element_rect(colour = "black", fill=NA, size = bord_size) else
	...
	theme_bw()+
		ggplot2::theme(
			...
			axis.text.x = ggplot2::element_text(size = base_size,  family = base_family, margin=ggplot2::margin(ifelse(ticks_type == "inner", 11, 5),5,10,5,"pt")),
			axis.text.y = ggplot2::element_text(size = base_size,  family = base_family, margin=ggplot2::margin(5,ifelse(ticks_type == "inner", 11, 5),10,5,"pt")),
			axis.title.y = ggplot2::element_text(size =  base_size,  vjust = 1.5, family = base_family),
			axis.title.x = ggplot2::element_text(size = base_size,vjust = -.5, family = base_family),
			...
			plot.title = ggplot2::element_text(face="bold",vjust = 2,  size = title_size, family = base_family),
